...
|Managed&nbsp;Spaces topology|When `managed_spaces` is enabled, `auto` selects one provider for the managed-mode session. A compatible scripting addition keeps the original synchronous topology path. With full SIP, the separate SIP-safe provider uses a validated SkyLight bridge or Mission Control Accessibility automation; Mission Control may briefly become visible, commands complete asynchronously, and the UI limit is 16 normal spaces per display.|
