...
const char *get_dock_spaces_pattern(NSOperatingSystemVersion os_version) {
    if (os_version.majorVersion == 27) {
        // doBindingCommand:display: on macOS 27.0 beta (26A5416b).
        return "C8 1E 00 B0 08 C1 2E 91 00 01 40 F9 E2 03 13 AA ?? ?? ?? 94 C8 1E 00 B0 08 A1 30 91 00 01 40 F9 ?? ?? ?? 94 00 05 00 34";
    } else if (os_version.majorVersion == 26) {
...
const char *get_set_front_window_pattern(NSOperatingSystemVersion os_version) {
    if (os_version.majorVersion == 27) {
        // References com.apple.window_proxies.startup; takes PSN and window id.
        return "?1 ?? ?? 34 7F 23 03 D5 FF ?? 01 D1 F6 ?? 04 A9 F4 ?? 05 A9 FD ?? 06 A9 FD ?? 01 91 F3 03 01 AA F5 03 00 AA";
    } else if (os_version.majorVersion == 26) {
