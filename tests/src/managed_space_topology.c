...
TEST_FUNC(managed_space_topology_backend_policy_values_are_stable,
{
    ...
    TEST_CHECK(managed_space_topology_backend_policy_from_string("sip-fallback", &policy), true);
    TEST_CHECK(policy, MANAGED_SPACE_TOPOLOGY_BACKEND_SIP_FALLBACK);
    ...
    TEST_CHECK(strcmp(managed_space_topology_provider_name(
...
TEST_FUNC(managed_space_topology_auto_selects_only_compatible_scripting_addition,
{
    ...
    TEST_CHECK(g_managed_space_sip_fallback.enabled, false);
    ...
    managed_space_topology_set_enabled(&topology, true);
    TEST_CHECK(topology.provider, MANAGED_SPACE_TOPOLOGY_PROVIDER_SIP_FALLBACK);
    TEST_CHECK(topology.selection_reason, MANAGED_SPACE_TOPOLOGY_SELECTION_HANDSHAKE_UNAVAILABLE);
    TEST_CHECK(g_managed_space_sip_fallback.enabled, true);
...
TEST_FUNC(managed_space_topology_forced_provider_never_cross_falls_back,
{
    ...
    managed_space_topology_set_enabled(&topology, false);
    topology.policy = MANAGED_SPACE_TOPOLOGY_BACKEND_SIP_FALLBACK;
    managed_space_topology_set_enabled(&topology, true);
    TEST_CHECK(topology.provider, MANAGED_SPACE_TOPOLOGY_PROVIDER_SIP_FALLBACK);
    TEST_CHECK(g_managed_space_sip_fallback.policy, MANAGED_SPACE_SIP_FALLBACK_POLICY_AUTO);

    managed_space_topology_set_enabled(&topology, false);
    topology.policy = MANAGED_SPACE_TOPOLOGY_BACKEND_ACCESSIBILITY;
    managed_space_topology_set_enabled(&topology, true);
    TEST_CHECK(topology.provider, MANAGED_SPACE_TOPOLOGY_PROVIDER_SIP_FALLBACK);
    TEST_CHECK(g_managed_space_sip_fallback.policy, MANAGED_SPACE_SIP_FALLBACK_POLICY_ACCESSIBILITY);

    managed_space_topology_set_enabled(&topology, false);
    managed_space_topology_destroy(&topology);
});

TEST_FUNC(managed_space_topology_primary_dispatch_is_synchronous_and_fallback_resources_stay_dormant,
{
    struct managed_space_topology topology;
    managed_space_topology_init(&topology);
    managed_space_topology_test_set_probe(SCRIPTING_ADDITION_PROBE_COMPATIBLE,
    ...
    TEST_CHECK(g_managed_space_sip_fallback.enabled, false);
    TEST_CHECK(g_managed_space_sip_fallback.state, MANAGED_SPACE_SIP_FALLBACK_STATE_IDLE);
    TEST_CHECK(g_managed_space_sip_fallback.ax_observer == NULL, true);
    TEST_CHECK(g_managed_space_sip_fallback.input_event_tap == NULL, true);
    TEST_CHECK(g_managed_space_sip_fallback.input_event_source == NULL, true);
    TEST_CHECK(managed_space_sip_fallback_bridge_symbols_resolved, false);
...
TEST_FUNC(managed_space_topology_selection_is_sticky_until_explicit_refresh,
{
    ...
    g_managed_space_sip_fallback.current.operation = MANAGED_SPACE_TOPOLOGY_OPERATION_CREATE;
    TEST_CHECK(managed_space_topology_set_backend_policy(&topology,
                                                        MANAGED_SPACE_TOPOLOGY_BACKEND_SCRIPTING_ADDITION),
               false);
    TEST_CHECK(topology.provider, MANAGED_SPACE_TOPOLOGY_PROVIDER_SIP_FALLBACK);
    g_managed_space_sip_fallback.current.operation = MANAGED_SPACE_TOPOLOGY_OPERATION_NONE;
