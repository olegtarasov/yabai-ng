...
*managed_spaces* ['<BOOL_SEL>']::
    ...
    managed mode. +
    With managed mode enabled, topology changes are serialized and may complete
    asynchronously. The scripting addition remains the primary backend. With
    full System Integrity Protection, yabai falls back to an exact-build
    validated SkyLight bridge or Mission Control Accessibility automation. The
    bridge is enabled automatically only after the operation agrees across SLS,
    Dock's Accessibility topology, persisted Dock state, and a Dock restart.
    Accessibility fallback briefly shows Mission Control and requires yabai to
    be enabled in System Settings -> Privacy & Security -> Accessibility. If the
    user already opened Mission Control, yabai waits for that session to end
    and does not close or control it. macOS exposes at most 16 normal spaces per
    display through Mission Control; reaching that limit blocks further managed
    creates until configuration or topology changes.
...
*managed_space_topology_backend* ['<auto|scripting-addition|bridge|accessibility>']::
    Select the managed-space topology backend. +
    `auto` is the default. It tries the scripting addition first, then a
    SkyLight bridge only when the exact operation has passed reversible
    validation on the current macOS build, and otherwise uses Accessibility.
    `scripting-addition`, `bridge`, and `accessibility` force a backend for
    diagnostics. Forced bridge mode bypasses the per-build validation table so
    an available bridge operation can be tested before it is added to that
    table. A successful SLS event alone is not sufficient validation because a
    bridge mutation can exist in WindowServer without being persisted by Dock.
    This setting affects topology changes only while
...
*--create*  ['<DISPLAY_SEL>']::
    ...
    With managed spaces enabled this command is queued through the configured
    topology backend and can run with full System Integrity Protection.
...
*--destroy* ['<SPACE_SEL>']::
    ...
    Managed membership is removed only after deletion is confirmed. With
    managed spaces enabled this command is asynchronous and can run with full
    System Integrity Protection. Otherwise, System Integrity Protection must be
    partially disabled.
...
*--move* '<SPACE_SEL>'::
    ...
    With managed spaces enabled this command is asynchronous, persists the
...
*--swap* '<SPACE_SEL>'::
    ...
    With managed spaces enabled, same-display topology changes are asynchronous,
    persist only after the full swap is confirmed, and can run with full System
    Integrity Protection. Otherwise, System Integrity Protection must be
    partially disabled.

*--display* '<DISPLAY_SEL>'::
    Send the selected space to the given display. +
    When managed spaces are enabled, sending a managed space to a display gives
    that managed space fixed display affinity after the move is confirmed. The
    managed command is asynchronous and can run with full System Integrity
    Protection. Otherwise, System Integrity Protection must be partially
    disabled.
...
Topology-changing commands return successfully and remain silent after a
request is safely queued. Completion is observable through this query and the
normal space and managed-space signals. An immediate validation failure is
returned to the caller. An asynchronous failure updates the
