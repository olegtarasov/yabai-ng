...
*managed_spaces* ['<BOOL_SEL>']::
    ...
    When turned on, yabai snapshots all current non-fullscreen user spaces as
    the managed set in Mission Control order. Future spaces created through
...
*managed_spaces_changed*::
    Triggered after managed mode reaches a stable reconciliation pass that
    changed managed topology or repaired window namespace state. +
