...
*managed_spaces* ['<BOOL_SEL>']::
    ...
    managed mode. +
    With managed mode enabled, `auto` selects one topology provider for the
    managed-mode session. An exact scripting-addition version with every
    required capability keeps the original synchronous scripting-addition
    behavior. With full System Integrity Protection, the separate SIP-safe
    provider serializes requests and uses an exact-build validated SkyLight
    bridge or Mission Control Accessibility automation. Provider selection is
    sticky: a Dock restart or operation failure never changes implementations
    implicitly. Reapply `managed_space_topology_backend` to explicitly
    re-evaluate `auto`. +
    ...
    blocks further SIP-safe creates until configuration or topology changes.
...
*managed_space_topology_backend* ['<auto|scripting-addition|bridge|accessibility>']::
    ...
    required capability selects the original synchronous scripting-addition
    provider; an unavailable, stale, or incomplete handshake selects the
    separate SIP-safe provider. The selection remains fixed until managed mode
    is disabled or this policy is explicitly reapplied. There is no implicit
    cross-provider fallback. +
    `scripting-addition` forces the synchronous provider and reports its normal
    immediate errors if the scripting addition cannot perform an operation.
    `bridge` and `accessibility` force the SIP-safe provider for diagnostics.
    Forced bridge mode bypasses the per-build validation table so an available
    bridge operation can be tested before it is added to that table. Within the
    SIP-safe provider, `auto` may fall back from a validated bridge operation to
    Accessibility only if bridge mutation has not begun. A successful SLS event
    alone is not sufficient validation because a bridge mutation can exist in
    WindowServer without being persisted by Dock. This setting affects topology
    changes only while `managed_spaces` is enabled.
...
*--create*  ['<DISPLAY_SEL>']::
    ...
    With managed spaces enabled, the scripting-addition provider preserves the
    original synchronous command. The SIP-safe provider queues this command and
...
*--destroy* ['<SPACE_SEL>']::
    ...
    With the SIP-safe provider, managed membership is removed only after
    deletion is confirmed, the command is asynchronous, and it can run with
    full System Integrity Protection. The scripting-addition provider preserves
    the original synchronous command. Otherwise, System Integrity Protection
...
*--move* '<SPACE_SEL>'::
    ...
    With the SIP-safe provider this command is asynchronous, persists the
    confirmed UUID order, and can run with full System Integrity Protection.
    The scripting-addition provider preserves the original synchronous command.
...
*--swap* '<SPACE_SEL>'::
    ...
    With the SIP-safe provider, same-display topology changes are asynchronous,
    persist only after the full swap is confirmed, and can run with full System
    Integrity Protection. The scripting-addition provider preserves the
    original synchronous command. Otherwise, System Integrity Protection must
...
*--display* '<DISPLAY_SEL>'::
    ...
    that managed space fixed display affinity. The SIP-safe provider commits
    that affinity after confirmation, runs asynchronously, and can work with
    full System Integrity Protection. The scripting-addition provider preserves
    the original synchronous command and commits affinity on return. Otherwise,
    System Integrity Protection must be partially disabled.
...
handshake result that selected it. With `scripting-addition`, topology commands
retain their original synchronous result and error behavior; SIP-safe queue
state stays idle and yabai does not own Mission Control. +

With `sip-safe`, topology-changing commands return successfully and remain
